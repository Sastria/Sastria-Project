    ...
    def run_VideoWriter(self,  isa_video, video_name):
		...
		for tr in isa_video._track_list :
			...
			try:
			    ...
			    frame_vid= self.cam.get(cv.CV_CAP_PROP_POS_FRAMES)
			    self.cam.set(cv.CV_CAP_PROP_POS_FRAMES,  bb._frame  )
			    frps = self.cam.get(cv.CV_CAP_PROP_FPS)
			    ret, frame = self.cam.read()
			    #img =  cv2.resize(frame, (frame.shape[1]/2, frame.shape[0]/2), interpolation = cv2.INTER_CUBIC)
			    img=frame
			    if writer ==None: 
				writer = cv2.VideoWriter(filename=video_name, fourcc=cv.CV_FOURCC('X','V','I','D'),fps=frps, frameSize=(frame.shape[1] , frame.shape[0] ))  
				print " creo il writer per generate video output ", writer  			    
			    imgout = None
			    imgout = img.copy()
			    x0,y0,x1,y1 = int(bb._position[0]), int(bb._position[1]), int(bb._position[2]), int(bb._position[3])
			    #print x0,y0,x1,y1
...
if __name__ == '__main__':
    ...
    print " VIDEOFACEREC ON QUOQUEISAALL"
    try:
	print " cerco db ", str(os.getcwd())+'/Data/db_shelve.db.db' 
