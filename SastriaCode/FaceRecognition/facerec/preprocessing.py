...
class Canny(AbstractFeature):
    def __init__(self, detected_edges=None,lowThreshold=30,lowThreshold_per_ratio=30*3,apertureSize = 3):
        AbstractFeature.__init__(self)
        self.detected_edges=detected_edges
        
    def compute(self,X,y):
        dst=None
        print "Canny compute"
        #print X
        if self.detected_edges==None :
            for xi in X:
                print xi
                xi = cv2.cvtColor(xi,cv2.COLOR_BGR2GRAY)
                gray = cv2.cvtColor(xi,cv2.COLOR_BGR2GRAY,  dstCn=1 )
                detected_edges = cv2.GaussianBlur(gray,(1,1),5)
                detected_edges = cv2.Canny(detected_edges,30,30*3, 3)
                dst = cv2.bitwise_and(xi,xi,mask = detected_edges)      
