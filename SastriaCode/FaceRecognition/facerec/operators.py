...
class ChainOperator3(FeatureOperator):
    """
    The ChainOperator chains two feature extraction modules:
        model2.compute(model1.compute(X,y),y)
    Where X can be generic input data.
    ...
    def compute(self,X,y):
        X = self.model1.compute(X,y)
        X = self.model2.compute(X,y)
        return self.model3.compute(X,y)
        
    def extract(self,X):
        X = self.model1.extract(X)
        X = self.model2.extract(X)
        return self.model3.extract(X)
    
    def __repr__(self):
        return "ChainOperator3(" + repr(self.model1) + "," + repr(self.model2) + ")"        
