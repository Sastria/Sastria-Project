...
bool TrackerKanade::trackf2f(const Mat& img1, const Mat& img2,vector<Point2f> &points1, vector<cv::Point2f> &points2){
  ...
  for(  i=k = 0; i<points2.size(); ++i ){
	   if((FB_status[i]==1)){
	  	  	  points1[k]=points1[i];
	  	  	  points2[k]=points2[i];
	  	  	  FB_error[k] = norm(pointsFB[i]-points1[i]);
	  	  	  k++;
	  }
  }
  points1.resize(k);
  ...
  normCrossCorrelationAle(img1,img2,points1,points2);
  return filterPtsAle(points1,points2);
}

void TrackerKanade::normCrossCorrelationAle(const Mat& img1,const Mat& img2, vector<Point2f>& points1, vector<Point2f>& points2) {
...
bool TrackerKanade::filterPtsAle(vector<Point2f>& points1,vector<Point2f>& points2){
  //Get Error Medians
  //simmed = medianAle(similarity);
	//int startPointsTFP=points1.size()*0.2;
...
/*
 * old OpenCV style
void TrackerKanade::init(Mat img0, vector<Point2f> &points){
  //Preallocate
